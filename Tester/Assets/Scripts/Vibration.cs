...
public static class Vibration
{
    ...
    public static void VibrateCustom ()
    {
        if ( Application.isMobilePlatform ) {
#if UNITY_IOS
            long[] pattern = { 0, 200 };
    ...
    public static bool HasFrequencyControl ()
    {
        if ( Application.isMobilePlatform ) {
            ...
            string Context_VIBRATOR_SERVICE = contextClass.GetStatic<string> ( "VIBRATOR_SERVICE" );
            AndroidJavaObject systemService = context.Call<AndroidJavaObject> ( "getSystemService", Context_VIBRATOR_SERVICE );
            if ( systemService.Call<bool> ( "hasFrequencyControl" ) ) {
                return true;
            } else {
                return false;
            }
    ...
    public static float getQFactor()
    {
        if (Application.isMobilePlatform)
        {
            ...
            string Context_VIBRATOR_SERVICE = contextClass.GetStatic<string> ( "VIBRATOR_SERVICE" );
            AndroidJavaObject systemService = context.Call<AndroidJavaObject> ( "getSystemService", Context_VIBRATOR_SERVICE );
            float qFactor = systemService.Call<float> ( "getQFactor" ) ;
            if (float.IsNaN(qFactor))
                return -30f;
            else
                return qFactor;
    ...
    public static float getResonantFrequency()
    {
        if (Application.isMobilePlatform)
        {
            ...
            string Context_VIBRATOR_SERVICE = contextClass.GetStatic<string> ( "VIBRATOR_SERVICE" );
            AndroidJavaObject systemService = context.Call<AndroidJavaObject> ( "getSystemService", Context_VIBRATOR_SERVICE );
            float resonantFrequency = systemService.Call<float> ( "getResonantFrequency" );
